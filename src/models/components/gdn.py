...
class GDN(nn.Module):
    def __init__(
        ...
        gama_init = torch.sqrt(
            torch.full(
                (num_features, num_features), fill_value=gamma_init, dtype=torch.float
            )
            * torch.eye(num_features, dtype=torch.float)
            + self.pedestal
        )
    ...
    def forward(self, x):
        gamma = self._reparam(self.gamma, self.gamma_bound).view(
