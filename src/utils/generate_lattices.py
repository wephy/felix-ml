...
for i, ICSD_code in enumerate(os.listdir(data_loc)):
    ...
    Cug = np.apply_along_axis(
        lambda row: complex(row[5], row[6]), 1, structure_factors)
    ...
    arr = (arr / np.max(arr))
    np.save(
        os.path.join(data_loc, ICSD_code, ICSD_code+"_structure"), arr)
    image.imsave(
        os.path.join(data_loc, ICSD_code, ICSD_code+"_structure.png"), arr,
        cmap="Greys")
    pattern = np.fromfile(
        os.path.join(data_loc, ICSD_code, ICSD_code+"_+0+0+0.bin"), dtype=np.float64)
    pattern = pattern.reshape((128, 128))
    np.save(
        os.path.join(data_loc, ICSD_code, ICSD_code+"_+0+0+0"), pattern)
    image.imsave(
        os.path.join(data_loc, ICSD_code, ICSD_code+"_+0+0+0.png"), pattern,
        cmap="Greys")
